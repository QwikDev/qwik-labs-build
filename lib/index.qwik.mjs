...
InsightsError._type;
const Insights = /* @__PURE__ */ componentQrl(/* @__PURE__ */ inlinedQrl((props) => {
...
function symbolTracker(window2, document2, location, navigator, publicApiKey, postUrl) {
  const sessionID = Math.random().toString(36).slice(2);
  const manifestHash = document2.querySelector("[q\\:manifest-hash]")?.getAttribute("q:manifest-hash") || "dev";
  const qSymbols = [];
  ...
  function flush() {
    ...
    if (qSymbols.length > flushSymbolIndex) {
      ...
      navigator.sendBeacon(postUrl.replace("${publicApiKey}", publicApiKey), JSON.stringify(payload));
  ...
  window2.addEventListener("error", (event) => {
    ...
    navigator.sendBeacon(postUrl.replace("${publicApiKey}", publicApiKey) + "error/", JSON.stringify(payload));
  });
}
...
function runQwikJsonDebug(window1, document1, debug) {
  const parseQwikJSON = () => {
    const rawData = JSON.parse(document1.querySelector('script[type="qwik/json"]').textContent);
    const derivedFns = document1.querySelector('script[q\\:func="qwik/json"]')?.qFuncs || [];
    const debugData = debug(document1, rawData, derivedFns);
    window1.qwikJson = debugData;
    console.log(debugData);
  };
  if (document1.querySelector('script[type="qwik/json"]'))
    parseQwikJSON();
  else
    document1.addEventListener("DOMContentLoaded", parseQwikJSON);
}
function qwikJsonDebug(document1, qwikJson, derivedFns) {
  class Base {
  ...
  function getNodeMap() {
    ...
    const walker = document1.createTreeWalker(document1.firstElementChild, NodeFilter.SHOW_COMMENT | NodeFilter.SHOW_ELEMENT);
